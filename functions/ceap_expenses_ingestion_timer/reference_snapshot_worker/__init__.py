"""Queue trigger: reference snapshot worker.

Consumes one message produced by ``reference_snapshot_dispatcher`` and runs the
full snapshot for that endpoint via :func:`shared.reference_run.run_reference_endpoint_snapshot`.
"""
...
def main(msg: func.QueueMessage) -> None:
    ...
    if wm.domain != domain.name:
        log_structured(
            logger,
            "warning",
            "Reference worker received message for a different domain.",
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Reference worker received unknown endpoint; dropping message.",
    ...
    reference_date = str(payload.get("reference_date", "")).strip()
    reference_timezone = str(
        payload.get("reference_timezone", "America/Sao_Paulo")
    )
    if not reference_date:
        log_structured(
            logger,
            "error",
            "Reference worker missing reference_date in payload.",
            pipeline_run_id=wm.pipeline_run_id,
            endpoint=wm.endpoint,
        )
        return
    ...
    state_row = _state_row_key(endpoint.name, reference_date)
    ...
    if same_run and str(state_now.get("status", "")).upper() == "SUCCESS":
        log_structured(
            logger,
            "info",
            "Reference worker skipped: endpoint already SUCCESS for this run.",
            domain=domain.name,
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Reference worker raised; queue may retry.",
            domain=domain.name,
            endpoint=endpoint.name,
            reference_date=reference_date,
            pipeline_run_id=wm.pipeline_run_id,
