"""Poison queue: mark reference partition POISON and increase failure counter."""
...
def main(msg: func.QueueMessage) -> None:
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Reference poison: could not parse message body.",
            error=str(exc)[:500],
        )
        return
    ...
    if wm.domain != domain.name:
        log_structured(
            logger,
            "warning",
            "Reference poison handler received message for a different domain.",
            received_domain=wm.domain,
            expected_domain=domain.name,
        )
        return

    reference_date = str((wm.payload or {}).get("reference_date", "")).strip()
    if not reference_date:
        log_structured(
            logger,
            "error",
            "Reference poison: missing reference_date in payload.",
    ...
    state_row = _state_row_key(wm.endpoint, reference_date)
    ...
    parts.upsert_partition(
        state_row,
        {
            ...
            "last_error": "Message landed on reference poison queue.",
