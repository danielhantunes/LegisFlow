"""Poison queue: mark institucional sub-endpoint partition POISON."""
...
def main(msg: func.QueueMessage) -> None:
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Institucional poison: could not parse message body.",
            error=str(exc)[:500],
        )
        return
    ...
    if wm.domain != domain.name:
        log_structured(
            logger,
            "warning",
            "Institucional poison handler received message for a different domain.",
    ...
    if not parent_id:
        log_structured(
            logger,
            "error",
            "Institucional poison: missing parent_id in payload.",
    ...
    state_row = _state_row_key(wm.endpoint, parent_id)
    ...
    parts.upsert_partition(
        state_row,
        {
            ...
            "last_error": "Message landed on institucional poison queue.",
