"""Poison queue: mark evento sub-endpoint partition POISON."""
...
def main(msg: func.QueueMessage) -> None:
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Eventos poison: could not parse message body.",
            error=str(exc)[:500],
        )
        return
    ...
    if wm.domain != domain.name:
        log_structured(
            logger,
            "warning",
            "Eventos poison handler received message for a different domain.",
    ...
    if not evento_id:
        log_structured(
            logger,
            "error",
            "Eventos poison: missing evento_id in payload.",
    ...
    state_row = _state_row_key(wm.endpoint, evento_id)
    ...
    parts.upsert_partition(
        state_row,
        {
            ...
            "last_error": "Message landed on eventos poison queue.",
