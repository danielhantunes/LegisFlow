"""Poison queue: mark votação partition POISON and increase failure counter."""
...
def main(msg: func.QueueMessage) -> None:
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Votacoes poison: could not parse message body.",
            error=str(exc)[:500],
        )
        return
    ...
    if wm.domain != domain.name:
        log_structured(
            logger,
            "warning",
            "Votacoes poison handler received message for a different domain.",
    ...
    if not votacao_id:
        log_structured(
            logger,
            "error",
            "Votacoes poison: missing votacao_id in payload.",
    ...
    parts.upsert_partition(
        state_row,
        {
            ...
            "last_error": "Message landed on votacoes poison queue.",
