"""Dev/test helper: reset institucional pipeline_run_id artifacts.

Mirrors :mod:`shared.eventos_pipeline_reset` but scoped to the institucional
domain. Guarded by either ``ENABLE_RESET_FUNCTIONS=true`` or
``ENABLE_INSTITUCIONAL_RESET_FUNCTION=true``.
"""
...
from .domain_catalog import INSTITUCIONAL_DOMAIN
from .institucional_pipeline_reset_helpers import (
...
def _count_or_delete_state_for_run(
    *, state_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    flt = (
        f"PartitionKey eq '{INSTITUCIONAL_DOMAIN.state_partition_key}' "
    ...
    *, control_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    try:
        control_tc.get_entity(
            partition_key=INSTITUCIONAL_DOMAIN.runs_partition_key,
    ...
    *, control_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    try:
        lock = control_tc.get_entity(
            partition_key=INSTITUCIONAL_DOMAIN.locks_partition_key,
            row_key=INSTITUCIONAL_DOMAIN.lock_row_key,
        )
    except ResourceNotFoundError:
        return 0, warnings
    ...
    if lock_pid != pipeline_run_id:
        warnings.append(
            f"Institucional dispatcher lock is for pipeline_run_id={lock_pid!r}; not cleared."
        )
        return 0, warnings
    if dry_run:
        return 1, warnings
    ...
    control_tc.upsert_entity(
        entity={
            "PartitionKey": INSTITUCIONAL_DOMAIN.locks_partition_key,
            "RowKey": INSTITUCIONAL_DOMAIN.lock_row_key,
            "locked_by": "",
            "locked_until": now,
            "pipeline_run_id": "",
            "updated_at": now,
...
def run_institucional_pipeline_reset(
    *,
    pipeline_run_id: str,
    dry_run: bool,
    delete_raw: bool,
    ...
    if delete_raw:
        ...
        prefixes = [
            "raw/camara/institucional/api/parents/",
            "raw/camara/institucional/api/orgaos/",
            "raw/camara/institucional/api/partidos/",
            "raw/camara/institucional/api/frentes/",
            "raw/camara/institucional/api/legislaturas/",
            "raw/camara/institucional/api/_metadata/",
        ]
