"""Dev/test helper: reset reference-snapshot pipeline_run_id artifacts.

Mirrors :mod:`shared.ceap_pipeline_reset` but scoped to the reference domain
(its own queue, partitions and Raw paths). Guarded by either
``ENABLE_RESET_FUNCTIONS=true`` (global) or
``ENABLE_REFERENCE_RESET_FUNCTION=true`` (domain-specific). Do not enable in
production.
"""
...
from .domain_catalog import REFERENCE_DOMAIN
from .queue_helpers import _queue_storage_connection_string
from .reference_pipeline_reset_helpers import (
...
def _count_or_delete_state_for_run(
    *,
    ...
    flt = (
        f"PartitionKey eq '{REFERENCE_DOMAIN.state_partition_key}' "
    ...
    *,
    ...
    try:
        control_tc.get_entity(
            partition_key=REFERENCE_DOMAIN.runs_partition_key,
    ...
    *,
    ...
    try:
        lock = control_tc.get_entity(
            partition_key=REFERENCE_DOMAIN.locks_partition_key,
            row_key=REFERENCE_DOMAIN.lock_row_key,
        )
    except ResourceNotFoundError:
        return 0, warnings
    ...
    if lock_pid != pipeline_run_id:
        warnings.append(
            f"Reference dispatcher lock is for pipeline_run_id={lock_pid!r}; not cleared."
        )
        return 0, warnings
    if dry_run:
        return 1, warnings
    ...
    control_tc.upsert_entity(
        entity={
            "PartitionKey": REFERENCE_DOMAIN.locks_partition_key,
            "RowKey": REFERENCE_DOMAIN.lock_row_key,
            "locked_by": "",
            "locked_until": now,
            "pipeline_run_id": "",
            "updated_at": now,
...
def run_reference_pipeline_reset(
    *,
    pipeline_run_id: str,
    dry_run: bool,
    delete_raw: bool,
    ...
    if delete_raw:
        ...
        for ep in REFERENCE_DOMAIN.endpoints:
            base_prefix = f"raw/camara/{ep.raw_prefix}/"
