"""Dev/test helper: reset eventos pipeline_run_id artifacts.
...
from .domain_catalog import EVENTOS_DOMAIN
from .eventos_pipeline_reset_helpers import (
...
def _count_or_delete_state_for_run(
    *, state_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    flt = (
        f"PartitionKey eq '{EVENTOS_DOMAIN.state_partition_key}' "
    ...
    *, control_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    try:
        control_tc.get_entity(
            partition_key=EVENTOS_DOMAIN.runs_partition_key,
    ...
    *, control_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    try:
        lock = control_tc.get_entity(
            partition_key=EVENTOS_DOMAIN.locks_partition_key,
            row_key=EVENTOS_DOMAIN.lock_row_key,
        )
    except ResourceNotFoundError:
        return 0, warnings
    ...
    if lock_pid != pipeline_run_id:
        warnings.append(
            f"Eventos dispatcher lock is for pipeline_run_id={lock_pid!r}; not cleared."
        )
        return 0, warnings
    if dry_run:
        return 1, warnings
    ...
    control_tc.upsert_entity(
        entity={
            "PartitionKey": EVENTOS_DOMAIN.locks_partition_key,
            "RowKey": EVENTOS_DOMAIN.lock_row_key,
            "locked_by": "",
            "locked_until": now,
            "pipeline_run_id": "",
            "updated_at": now,
...
def run_eventos_pipeline_reset(
    *,
    pipeline_run_id: str,
    dry_run: bool,
    delete_raw: bool,
    ...
    if delete_raw:
        ...
        prefixes = [
            "raw/camara/eventos/api/list/",
            "raw/camara/eventos/api/deputados/",
            "raw/camara/eventos/api/orgaos/",
            "raw/camara/eventos/api/pauta/",
            "raw/camara/eventos/api/votacoes/",
            "raw/camara/eventos/api/_metadata/",
        ]
