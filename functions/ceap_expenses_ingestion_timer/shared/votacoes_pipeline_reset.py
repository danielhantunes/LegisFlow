"""Dev/test helper: reset votacoes pipeline_run_id artifacts.

Mirrors :mod:`shared.reference_pipeline_reset` but scoped to the votacoes
domain (its own queue, partitions and Raw paths). Guarded by either
``ENABLE_RESET_FUNCTIONS=true`` (global) or
``ENABLE_VOTACOES_RESET_FUNCTION=true`` (domain-specific). Do not enable in
production.
"""
...
from .domain_catalog import VOTACOES_DOMAIN
from .queue_helpers import _queue_storage_connection_string
from .votacoes_pipeline_reset_helpers import (
...
def _count_or_delete_state_for_run(
    *, state_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    flt = (
        f"PartitionKey eq '{VOTACOES_DOMAIN.state_partition_key}' "
    ...
    *, control_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    try:
        control_tc.get_entity(
            partition_key=VOTACOES_DOMAIN.runs_partition_key,
    ...
    *, control_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    try:
        lock = control_tc.get_entity(
            partition_key=VOTACOES_DOMAIN.locks_partition_key,
            row_key=VOTACOES_DOMAIN.lock_row_key,
        )
    except ResourceNotFoundError:
        return 0, warnings
    ...
    if lock_pid != pipeline_run_id:
        warnings.append(
            f"Votacoes dispatcher lock is for pipeline_run_id={lock_pid!r}; not cleared."
        )
        return 0, warnings
    if dry_run:
        return 1, warnings
    ...
    control_tc.upsert_entity(
        entity={
            "PartitionKey": VOTACOES_DOMAIN.locks_partition_key,
            "RowKey": VOTACOES_DOMAIN.lock_row_key,
            "locked_by": "",
            "locked_until": now,
            "pipeline_run_id": "",
            "updated_at": now,
...
def run_votacoes_pipeline_reset(
    *,
    pipeline_run_id: str,
    dry_run: bool,
    delete_raw: bool,
    ...
    if delete_raw:
        ...
        # Scan votacoes/api/* (list + votos + _metadata for the aggregate run)
        prefixes = [
            "raw/camara/votacoes/api/list/",
            "raw/camara/votacoes/api/votos/",
            "raw/camara/votacoes/api/_metadata/",
        ]
