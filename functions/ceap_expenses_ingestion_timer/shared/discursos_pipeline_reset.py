"""Dev/test helper: reset discursos pipeline_run_id artifacts.
...
from .discursos_pipeline_reset_helpers import (
    message_matches_pipeline_run,
    safe_path_segment,
)
from .domain_catalog import DISCURSOS_DOMAIN
...
def _count_or_delete_state_for_run(
    *, state_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    flt = (
        f"PartitionKey eq '{DISCURSOS_DOMAIN.state_partition_key}' "
    ...
    *, control_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    try:
        control_tc.get_entity(
            partition_key=DISCURSOS_DOMAIN.runs_partition_key,
    ...
    *, control_tc: Any, pipeline_run_id: str, dry_run: bool
    ...
    try:
        lock = control_tc.get_entity(
            partition_key=DISCURSOS_DOMAIN.locks_partition_key,
            row_key=DISCURSOS_DOMAIN.lock_row_key,
        )
    except ResourceNotFoundError:
        return 0, warnings
    ...
    if lock_pid != pipeline_run_id:
        warnings.append(
            f"Discursos dispatcher lock is for pipeline_run_id={lock_pid!r}; not cleared."
        )
        return 0, warnings
    if dry_run:
        return 1, warnings
    ...
    control_tc.upsert_entity(
        entity={
            "PartitionKey": DISCURSOS_DOMAIN.locks_partition_key,
            "RowKey": DISCURSOS_DOMAIN.lock_row_key,
            "locked_by": "",
            "locked_until": now,
            "pipeline_run_id": "",
            "updated_at": now,
...
def run_discursos_pipeline_reset(
    *,
    pipeline_run_id: str,
    dry_run: bool,
    delete_raw: bool,
