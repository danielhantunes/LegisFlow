"""HTTP: controlled reset of one votacoes pipeline_run_id (dev/test only).

Requires either ``ENABLE_RESET_FUNCTIONS=true`` or
``ENABLE_VOTACOES_RESET_FUNCTION=true``.
"""
...
from shared.votacoes_pipeline_reset import (
    ResetSummary,
    run_votacoes_pipeline_reset,
)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not reset_enabled_for_domain(domain.reset_feature_flag_env):
        log_structured(
            logger,
            "warning",
            "Votacoes pipeline reset rejected (feature disabled).",
    ...
    if not is_allowed_votacoes_pipeline_run_id(pipeline_run_id):
        return _json_response(
            {
                "error": (
                    "pipeline_run_id must match votacoes_microbatch_YYYYMMDDHHMM "
                    "or votacoes_reconciliation_YYYYMMDD."
                )
            },
            status=400,
        )
    ...
    queue_work = os.getenv("VOTACOES_QUEUE_NAME", domain.queue_work)
    queue_poison = os.getenv(
        "VOTACOES_POISON_QUEUE_NAME", domain.queue_poison
    )
    raw_account = os.environ["RAW_STORAGE_ACCOUNT_NAME"]
    filesystem = os.getenv("LAKEHOUSE_FILESYSTEM_NAME", "lakehouse")

    try:
        summary: ResetSummary = run_votacoes_pipeline_reset(
        ...
        log_structured(
            logger,
            "error",
            "Votacoes pipeline reset failed.",
            pipeline_run_id=pipeline_run_id,
            dry_run=dry_run,
            error=str(exc)[:2048],
    ...
    log_structured(
        logger,
        "info",
        "Votacoes pipeline reset finished.",
        pipeline_run_id=pipeline_run_id,
        dry_run=dry_run,
