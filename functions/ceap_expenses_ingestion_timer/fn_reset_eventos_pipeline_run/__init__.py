"""HTTP: controlled reset of one eventos pipeline_run_id (dev/test only).
...
from shared.eventos_pipeline_reset import (
    ResetSummary,
    run_eventos_pipeline_reset,
)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not reset_enabled_for_domain(domain.reset_feature_flag_env):
        log_structured(
            logger,
            "warning",
            "Eventos pipeline reset rejected (feature disabled).",
    ...
    if not is_allowed_eventos_pipeline_run_id(pipeline_run_id):
        return _json_response(
            {
                "error": (
                    "pipeline_run_id must match eventos_daily_YYYYMMDD, "
                    "eventos_microbatch_YYYYMMDDHHMM, or eventos_reconciliation_YYYYMMDD."
                )
            },
            status=400,
        )
    ...
    queue_work = os.getenv("EVENTOS_QUEUE_NAME", domain.queue_work)
    queue_poison = os.getenv(
        "EVENTOS_POISON_QUEUE_NAME", domain.queue_poison
    )
    raw_account = os.environ["RAW_STORAGE_ACCOUNT_NAME"]
    filesystem = os.getenv("LAKEHOUSE_FILESYSTEM_NAME", "lakehouse")

    try:
        summary: ResetSummary = run_eventos_pipeline_reset(
    ...
    log_structured(
        logger,
        "info",
        "Eventos pipeline reset finished.",
        pipeline_run_id=pipeline_run_id,
        dry_run=dry_run,
