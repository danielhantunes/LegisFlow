...
def main(msg: func.QueueMessage) -> None:
    ...
    if wm.domain != domain.name:
        log_structured(
            logger,
            "warning",
            "Votacoes worker received message for a different domain.",
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Votacoes worker received unknown endpoint; dropping message.",
            endpoint=wm.endpoint,
            pipeline_run_id=wm.pipeline_run_id,
        )
        return

    if endpoint.name != "votacao_votos":
        log_structured(
            logger,
            "warning",
            "Votacoes worker only handles 'votacao_votos' endpoint.",
    ...
    if not votacao_id:
        log_structured(
            logger,
            "error",
            "Votacoes worker missing votacao_id in payload.",
            pipeline_run_id=wm.pipeline_run_id,
        )
        return
    ...
    def _fetch(page: int) -> tuple[dict, int]:
        return api.list_votacao_votos_page(
            votacao_id, page=page, itens=endpoint.items_per_page
        )
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Votacoes worker raised; queue may retry.",
            domain=domain.name,
            endpoint=endpoint.name,
            votacao_id=votacao_id,
            pipeline_run_id=wm.pipeline_run_id,
