"""HTTP: controlled reset of one discursos pipeline_run_id (dev/test only).
...
from shared.discursos_pipeline_reset import (
    ResetSummary,
    run_discursos_pipeline_reset,
)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not reset_enabled_for_domain(domain.reset_feature_flag_env):
        log_structured(
            logger,
            "warning",
            "Discursos pipeline reset rejected (feature disabled).",
    ...
    if not is_allowed_discursos_pipeline_run_id(pipeline_run_id):
        return _json_response(
            {
                "error": (
                    "pipeline_run_id must match discursos_daily_YYYYMMDD, "
                    "discursos_reconciliation_YYYYMMDD, or discursos_microbatch_YYYYMMDDHHMM."
                )
            },
            status=400,
        )
    ...
    queue_work = os.getenv("DISCURSOS_QUEUE_NAME", domain.queue_work)
    queue_poison = os.getenv(
        "DISCURSOS_POISON_QUEUE_NAME", domain.queue_poison
    )
    raw_account = os.environ["RAW_STORAGE_ACCOUNT_NAME"]
    filesystem = os.getenv("LAKEHOUSE_FILESYSTEM_NAME", "lakehouse")

    try:
        summary: ResetSummary = run_discursos_pipeline_reset(
        ...
        log_structured(
            logger,
            "error",
            "Discursos pipeline reset failed.",
            pipeline_run_id=pipeline_run_id,
            dry_run=dry_run,
            error=str(exc)[:2048],
    ...
    log_structured(
        logger,
        "info",
        "Discursos pipeline reset finished.",
        pipeline_run_id=pipeline_run_id,
        dry_run=dry_run,
