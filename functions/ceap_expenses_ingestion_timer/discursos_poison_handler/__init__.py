"""Poison queue: mark discursos partition POISON."""
...
def main(msg: func.QueueMessage) -> None:
    ...
    try:
        ...
        log_structured(
            logger,
            "error",
            "Discursos poison: could not parse message body.",
            error=str(exc)[:500],
        )
        return
    ...
    if wm.domain != domain.name:
        log_structured(
            logger,
            "warning",
            "Discursos poison handler received message for a different domain.",
    ...
    if not deputado_id:
        log_structured(
            logger,
            "error",
            "Discursos poison: missing deputado_id in payload.",
    ...
    parts.upsert_partition(
        state_row,
        {
            ...
            "last_error": "Message landed on discursos poison queue.",
