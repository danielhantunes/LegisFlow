"""HTTP: controlled reset of one reference pipeline_run_id (dev/test only).

Requires either ``ENABLE_RESET_FUNCTIONS=true`` or
``ENABLE_REFERENCE_RESET_FUNCTION=true``.
"""
...
from shared.reference_pipeline_reset import (
    ResetSummary,
    run_reference_pipeline_reset,
)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not reset_enabled_for_domain(domain.reset_feature_flag_env):
        log_structured(
            logger,
            "warning",
            "Reference pipeline reset rejected (feature disabled).",
    ...
    if not is_allowed_reference_pipeline_run_id(pipeline_run_id):
        return _json_response(
            {
                "error": (
                    "pipeline_run_id must match reference_snapshot_YYYYMMDD."
                )
            },
            status=400,
        )
    ...
    queue_work = os.getenv("REFERENCE_SNAPSHOT_QUEUE_NAME", domain.queue_work)
    queue_poison = os.getenv(
        "REFERENCE_SNAPSHOT_POISON_QUEUE_NAME", domain.queue_poison
    )
    raw_account = os.environ["RAW_STORAGE_ACCOUNT_NAME"]
    filesystem = os.getenv("LAKEHOUSE_FILESYSTEM_NAME", "lakehouse")

    try:
        summary: ResetSummary = run_reference_pipeline_reset(
        ...
        log_structured(
            logger,
            "error",
            "Reference pipeline reset failed.",
            pipeline_run_id=pipeline_run_id,
            dry_run=dry_run,
            error=str(exc)[:2048],
    ...
    log_structured(
        logger,
        "info",
        "Reference pipeline reset finished.",
        pipeline_run_id=pipeline_run_id,
        dry_run=dry_run,
