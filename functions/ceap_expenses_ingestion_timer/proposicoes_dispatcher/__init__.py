...
3. Lists ``/proposicoes`` over ``[now - lookback, now]`` (``dataInicio`` /
   ``dataFim`` filter on the API which targets the **last tramitação update**
   field, exactly the watermark we care about).
    ...
    try:
        ...
        for pid in sorted(detected_ids):
            ...
            for sub_ep in (autores_ep, tramitacoes_ep):
                if enqueued_now >= max_messages_per_tick:
                    break
                row = _state_row_key(sub_ep.name, pid)
