...
def main(timer: func.TimerRequest) -> None:
    ...
    log_structured(logger, "info", "Starting CEAP ingestion.", pipeline_run_id=pipeline_run_id, execution_id=execution_id)
    try:
...
def _run_pipeline(pipeline_run_id: str, execution_id: str, state_store: IngestionStateStore) -> None:
    ...
    log_structured(
        logger,
        "info",
        "Raw ingestion stage complete; Databricks trigger placeholder reached.",
        pipeline_run_id=pipeline_run_id,
    )
...
def _ingest_deputies(api: CamaraApiClient, raw_writer: AdlsRawWriter, pipeline_run_id: str, execution_id: str) -> list[int]:
    ...
    while True:
        payload = api.list_deputies_page(page=page)
        path = (
...
def _process_partition(
    ...
    try:
        while True:
            payload = api.list_expenses_page(deputy_id=deputy_id, ano=ano, mes=mes, page=page)
            path = (
